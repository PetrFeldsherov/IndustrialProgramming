...
public class AlgExprProceeder { // плохо еще то что у меня пакедж хелперс, можно сделать утилз, в котором будут
				// конвертера два и сериализейшн, в котором будут фактори по письму и парсингу,
				// еще надо переименовать громоздкие креате
	...
	convertor = new Convertor(srcFormat, destFormat); // перевод в строку это просто обычный утилз с подгоном через
							  // свитч сделать, перевод в файл это нужно перегрузку
							  // райтТуФайл слделать и тогда не так уродски с иерархией
							  // будет помимо прочего
    }
	...
	if (destFormat == FormatType.TXT || srcFormat == destFormat) {
	    String srcAsString = convertor.fileToString(src); // должен быть статик метод со вторым параметром
							      // FormatType
	    srcAsString = passResults(srcAsString);
	    writer.writeToFile(destFilename, srcAsString); // TODO write to dest as string, только неясно получить ли
							   // мне сам хмл нужно или еще что. пока что все очень плохо, я
							   // навешал функционала, который думал, что стройно выстроится
							   // и быстро закодируется, а пока что сплошные траблы, вот как
							   // я надеялся записать в файл нечто, короче тут надо получить
							   // результат и прописать его в какое-то место в памяти
							   // дополнительным методом, страшно
	...
	while (!expressions.isEmpty()) {
	    ...
	    src.replaceAll(expression.toConditionNode(srcFormat), expression.toResultNode(srcFormat)); // тоже не самая
												       // тривиальная
												       // задача, судя
												       // по всему нужно
												       // просто сделать
